...
class SceneformView extends React.Component {
  ...
  onChange(event) {
    ...
    if (event.nativeEvent.onFeatureMapQualityChange) {
      ...
      this.props.onFeatureMapQualityChange({
        onFeatureMapQualityChange: event.nativeEvent,
      });
    }
  ...
  componentDidMount() {
    ...
    if (this.props.onAnchorHost) {
      let sub = DeviceEventEmitter.addListener(
        'onAnchorHost',
        this.props.onAnchorHost
      );
      this.subscriptions.push(sub);
    }
    if (this.props.onFeatureMapQualityChange) {
      let sub = DeviceEventEmitter.addListener(
        'onFeatureMapQualityChange',
        this.props.onFeatureMapQualityChange
      );
      this.subscriptions.push(sub);
    }
  ...
  hostCloudAnchor = (anchorId) => {
    NativeModules.SceneformModule.hostCloudAnchor(
      findNodeHandle(this),
      parseInt(anchorId, 10)
    );
  };

  resolveCloudAnchor = (anchorId) => {
    NativeModules.SceneformModule.resolveCloudAnchor(
      findNodeHandle(this),
      anchorId
    );
  };
  ...
  startVideoRecording = async () => {
    ...
    const recording = await NativeModules.SceneformModule.startVideoRecording(
      findNodeHandle(this)
    );
    if (recording) {
  ...
  stopVideoRecording = () => {
    ...
    return NativeModules.SceneformModule.stopVideoRecording(
      findNodeHandle(this)
    );
  };
